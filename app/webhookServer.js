...
function verifyTrelloWebhookRequest(hostname, body, signature) {
  ...
  const base64Digest = s => crypto.createHmac('sha1', process.env.TRELLO_CLIENT_SECRET).update(s).digest('base64');
  const content = body + hostname;
...
class TrelloWebhookServer {
  ...
  start() {
    return new Promise((resolve, reject) => {
      this._server = http.createServer((req, response) => {
        const res = response;
